document.addEventListener('DOMContentLoaded', () => {
    ...
    generateBtn.addEventListener('click', async () => {
        if (!contentInput.value.trim()) {
            showNotification('Please enter content to hash', 'error');
            return;
        }

        try {
            const algorithm = algorithmSelect.value;
            const fullHash = await generateHash(contentInput.value, algorithm);
    ...
    async function generateHash(content, algorithm) {
        // Convert string to lowercase to handle the algorithm names
        const algoLower = algorithm.toLowerCase();
        
        // Use crypto.subtle for SHA algorithms
        if (algoLower.startsWith('sha')) {
            // Handle different SHA algorithms
            let cryptoAlgorithm;
            
            if (algoLower === 'sha-1') {
                cryptoAlgorithm = 'SHA-1';
            } else if (algoLower === 'sha-256') {
                cryptoAlgorithm = 'SHA-256';
            } else if (algoLower === 'sha-384') {
                cryptoAlgorithm = 'SHA-384';
            } else if (algoLower === 'sha-512') {
                cryptoAlgorithm = 'SHA-512';
            } else {
                throw new Error('Unsupported SHA algorithm');
            }
        ...
        // For MD5, we need to implement it ourselves since it's not in Web Crypto API
    ...
    // MD5 implementation (since it's not available in Web Crypto API)
    function md5(string) {
        // This is a simple implementation for demonstration
        // In production, you should use a well-tested library
